...
double GaussKernels::m(double xi) const {
    if (limit_m_ != -1 && xi > limit_m_) {
        return 0;
    }
    return std::exp(-xi * xi / (2 * sigma_m_ * sigma_m_)) / sqrt(2 * 3.1416 * sigma_m_ * sigma_m_);
}
double GaussKernels::w(double xi) const {
    if (limit_w_ != -1 && xi > limit_w_) {
        return 0;
    }
    return std::exp(-xi * xi / (2 * sigma_w_ * sigma_w_)) / sqrt(2 * 3.1416 * sigma_w_ * sigma_w_);
}